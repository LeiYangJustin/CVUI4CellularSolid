...
void Scene::collect_visible_points(std::vector<Point_2>& points) const
{
    for (unsigned i = 0; i < m_vertices.size(); ++i)
    {
        Vertex_handle vi = m_vertices[i];
...
void Scene::collect_sites(std::vector<Point_2>& points,
                          std::vector<FT>& weights) const
{
    for (unsigned i = 0; i < m_vertices.size(); ++i)
    {
        Vertex_handle vi = m_vertices[i];
        Point_2 pi = vi->get_position();
        points.push_back(pi);
...
void Scene::construct_triangulation(const std::vector<Point_2>& points,
...
void Scene::populate_vertices(const std::vector<Point_2>& points,
                              const std::vector<FT>& weights)
{
    //if (m_timer_on)
...
Vertex_handle Scene::insert_vertex(const Point_2& point,
                                   const FT weight,
...
void Scene::update_positions(const std::vector<Point_2>& points, bool clamp, bool hidden)
{
    unsigned j = 0;
    for (unsigned i = 0; i < m_vertices.size(); ++i)
    {

...
void CReconstructor::Update(CVoronoiDiagram *vd)
{
	...
	typedef std::set<CReconstructionPoint, CReconstructionPoint::point_less_compare> RPointSet;
	std::map<CReconstructionEdge, RPointSet, CReconstructionEdge::edge_less_compare> map_edges_to_rpset;
	for (RT_Finite_Edge_Iter eit = rt->finite_edges_begin(); eit != rt->finite_edges_end(); ++eit)
	{
		...
		// FIX THIS ERROR
		Point2 pstart = fh->vertex((vind+1)%3)->point().point();
		Point2 pend = fh->vertex((vind+2)%3)->point().point();
	...
	{
		Point2 fitp1, fitp2;
		update_edge_by_fitting(fitp1, fitp2, miter->second);
...
void CReconstructor::update_edge_by_fitting(Point2 &fitp1, Point2 &fitp2,

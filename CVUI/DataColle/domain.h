...
public:
	...
	{
		if (std::fabs(p.x()) > width()) return false;
		if (std::fabs(p.y()) > height()) return false;
		return true;
	}
	...
	{
		int side = -1;
		if (std::abs(a.x() - width()) < MY_EPS) side = 1;
		if (std::abs(a.x() + width()) < MY_EPS) side = 3;
		if (std::abs(a.y() - height()) < MY_EPS) side = 2;
		if (std::abs(a.y() + height()) < MY_EPS) side = 0;
		return side;
	}
